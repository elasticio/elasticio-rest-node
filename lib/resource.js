...
ApiResource.method = function method(spec) {
    ...
    function provideBody(verb, args) {
        ...
        function getArgumentByIndex(index, message) {
            var result = args.length === index + 1 && args[index];
        ...
        if (verb === ApiResource.POST) {

            return getArgumentByIndex(0, 'You must provide 1st argument to create new object');
        }

        if (verb === ApiResource.PUT) {
            return getArgumentByIndex(1, 'You must provide 2nd argument to update  object by given ID');
        }
