...
function ApiResource(client, options = {}) {
    this.client = client;
...
ApiResource.method = function method(spec) {
    ...
    async function sendRequest(...args) {
        ...
        function provideOptions() {
            ...
            return createRequestOptions(verb, api, path, requestBody, Object.assign({}, self.options, options));
        }
    ...
    function createRequestOptions(verb, api, path, body, { retryOnNetworkError, retryDelay, maxAttempts } = {}) {
        var options = {
            url: util.format("%s/%s", api.basePath, path),
            method: verb,
            json: true,
        ...
        if (retryOnNetworkError || _.isUndefined(retryOnNetworkError)) {
